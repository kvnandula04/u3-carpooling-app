...
const PlanTrip = ({ preferenceData }) => {
  ...
  const myUserLocation = useSelector(
    (state) => state.mySlice.startLocationText
  );

  var today = new Date();
  var departure_time = today.toLocaleTimeString("en", {
    hour: "numeric",
    hour12: false,
    minute: "numeric",
  });
        ...
        <View id="planTripTitleView" style={[styles.planTripTitleView]}>
          <Text
            style={[
              ...
              myUserRole == 0 ? { color: cream } : { color: yellow },
              // {
              //     color: mainColour,
