...
const LiveMap = (props) => {
    ...
    const gJourneyDuration = useSelector(
        (state) => state.mySlice.gJourneyDuration
    );
    ...
    const [origin, setOrigin] = useState(
        props.showRoute
            ...
            : { latitude: 51.38156107044501, longitude: -2.359066572043425 }
    );
    const [destination, setDestination] = useState(
        props.showRoute
            ...
            : { latitude: 51.37943205963418, longitude: -2.3256547832841625 }
    );

    const [currentUserLocation, setCurrentUserLocation] = useState();

    useEffect(() => {
        dispatch(updateJourneyDuration(duration));
    }, [duration]);

    useEffect(() => {
        dispatch(updateJourneyDistance(distance));
    }, [distance]);
    ...
    useEffect(() => {
        ...
        percentageLeft = d / gJourneyDistance;
        dispatch(updateETA(gJourneyDuration * percentageLeft));
