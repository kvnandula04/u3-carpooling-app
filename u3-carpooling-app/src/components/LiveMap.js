...
const LiveMap = (props) => {
  ...
  if (!callLocation && currentUserLocation) setCallLocation(true);
  ...
  const _getLocationAsync = async () => {
    this.location = await Location.watchPositionAsync(
      ...
      (newLocation) => {
        ...
        if (!currentUserLocation) setCurrentUserLocation(coords);
      },
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!currentUserLocation) setCurrentUserLocation(location);
  ...
  function traceRouteOnReady(args) {
    if (args) {
      ...
      dispatch(updateJourneyDistance(args.distance));
      setDuration(args.duration);
      dispatch(updateETA(args.duration));
    }
  }
